...
public class OnEnemyKillPatch : ModulePatch
{
    ...
    {
        return AccessTools.Method(typeof(EFT.BaseStatisticsManager),
            nameof(EFT.BaseStatisticsManager.OnEnemyKill));
    }

    [PatchPostfix]
    private static void Postfix(Player ___player_0, EFT.Ballistics.DamageInfo damage)
    {
        ...
        {
            var isSuppressed = weap.Mods.Any(x => x is EFT.InventoryLogic.Silencer);
