...
internal class SkillManagerConstructorPatch : ModulePatch
{
    ...
    [PatchPostfix]
    public static void Postfix(SkillManager __instance, ref EFT.Skill[] ___DisplayList, ref EFT.Skill[] ___Skills)
    {
        // Idempotency guard: if new skills already injected, skip
        if (__instance.UsecArsystems is not null) return;

        var insertIndex = 12;
        
        // Action and buff lists intentionally empty here, this is for display list purposes only
        
        __instance.UsecArsystems = new EFT.Skill(
            __instance, 
        ...
        __instance.BearAksystems = new EFT.Skill(
            __instance, 
        ...
        var newDisplayList = new EFT.Skill[___DisplayList.Length + 5];
        ...
        AccessTools.Field(typeof(EFT.Skill), "Locked").SetValue(__instance.UsecArsystems,
            !SkillsPlugin.SkillData.NatoRifle.Enabled);

        AccessTools.Field(typeof(EFT.Skill), "Locked").SetValue(__instance.BearAksystems,
            !SkillsPlugin.SkillData.EasternRifle.Enabled);
        
        AccessTools.Field(typeof(EFT.Skill), "Locked").SetValue(__instance.Lockpicking,
            !SkillsPlugin.SkillData.LockPicking.Enabled);
        
        AccessTools.Field(typeof(EFT.Skill), "Locked").SetValue(__instance.FieldMedicine,
            !SkillsPlugin.SkillData.FieldMedicine.Enabled);

        AccessTools.Field(typeof(EFT.Skill), "Locked").SetValue(__instance.FirstAid,
            !SkillsPlugin.SkillData.FirstAid.Enabled);
        
        AccessTools.Field(typeof(EFT.Skill), "Locked").SetValue(__instance.ProneMovement,
            !SkillsPlugin.SkillData.ProneMovement.Enabled);
        
        AccessTools.Field(typeof(EFT.Skill), "Locked").SetValue(__instance.SilentOps,
