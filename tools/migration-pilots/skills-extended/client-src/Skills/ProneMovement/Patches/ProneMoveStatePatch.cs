...
public class ProneMoveStatePatch : ModulePatch
{
    ...
    private static float _lastProneXpTime;  // Throttle XP to reduce per-frame GC
    ...
    private static bool Prefix(MovementContext __instance, float speed, ref float __result)
    {
        ...
        if (__instance.CurrentState is not EFT.ProneMovePlayerState) return true;
        ...
        if (!player.Skills.ProneMovement.IsEliteLevel
            && Time.time - _lastProneXpTime > 0.5f)  // Grant XP every 0.5s instead of every frame
        {
            var xp = ProneData.XpPerAction;
            var localPlayer = player;
            player.ExecuteSkill(() => localPlayer.Skills.ProneAction.Complete(xp));
            _lastProneXpTime = Time.time;
        }
