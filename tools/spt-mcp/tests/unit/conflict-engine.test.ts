...
describe("conflict-engine: isCompatibleWithTarget 版本语义", () => {
  ...
  it("目标为 major.minor 时 ~ 视为系列匹配（3.11.3 兼容目标 3.11）", () => {
    expect(isCompatibleWithTarget("~3.11.3", "3.11")).toBe(true);
    expect(isCompatibleWithTarget("~3.11.3", "3.11.4")).toBe(true);
    expect(isCompatibleWithTarget("~3.11.3", "3.12")).toBe(false);
    expect(isCompatibleWithTarget("~3.10.x", "3.11")).toBe(false);
  });

  it("目标显式带 patch 时 ~ 按 >=patch 严格匹配", () => {
    expect(isCompatibleWithTarget("~3.11.3", "3.11.2")).toBe(false);
    expect(isCompatibleWithTarget("~3.11.3", "3.11.5")).toBe(true);
  });
});
...
describe("conflict-engine: 文件覆盖检测", () => {
  it("结构性公共文件（package.json/meta.ini/src/mod.ts）不报覆盖冲突", () => {
    const a = serverMod({ path: "/m/a", name: "A", guid: "com.a" });
    const b = serverMod({ path: "/m/b", name: "B", guid: "com.b" });
    const report = analyzeConflicts({
      mods: [a, b],
      filesByModPath: {
        "/m/a": [
          { relativePath: "package.json", sizeBytes: 1 },
          { relativePath: "meta.ini", sizeBytes: 2 },
          { relativePath: "src/mod.ts", sizeBytes: 3 },
          { relativePath: "db/base.json", sizeBytes: 4 },
        ],
        "/m/b": [
          { relativePath: "package.json", sizeBytes: 5 },
          { relativePath: "meta.ini", sizeBytes: 6 },
          { relativePath: "src/mod.ts", sizeBytes: 7 },
          { relativePath: "db/base.json", sizeBytes: 8 },
        ],
      },
    });
    const overwrites = report.sections.O.filter((f) => f.kind === "file_overwrite");
    // package.json/meta.ini/src/mod.ts 被忽略；db/base.json 保留
    const paths = overwrites.map((f) => f.detail?.relativePath);
    expect(paths).toContain("db/base.json");
    expect(paths).not.toContain("package.json");
    expect(paths).not.toContain("meta.ini");
    expect(paths).not.toContain("src/mod.ts");
  });
  ...
  it("同路径文件内容一致 -> 降级为 duplicate_file（无害重复）", () => {
    const report = run({
      mods: [
        serverMod({ path: "/mods/a", name: "A" }),
        serverMod({ path: "/mods/b", name: "B" }),
      ...
      filesByModPath: {
        "/mods/a": [{ relativePath: "db/base.json", sizeBytes: 10, contentHash: "abc123" }],
        "/mods/b": [{ relativePath: "db/base.json", sizeBytes: 10, contentHash: "abc123" }],
      },
    });
    const dup = report.sections.O.filter((f) => f.kind === "duplicate_file");
    const ow = report.sections.O.filter((f) => f.kind === "file_overwrite");
    expect(dup).toHaveLength(1);
    expect(dup[0].detail?.contentIdentical).toBe(true);
    expect(ow).toHaveLength(0);
  });

  it("同路径文件内容不同 -> 保留 file_overwrite（真冲突）", () => {
    const report = run({
      mods: [
        serverMod({ path: "/mods/a", name: "A" }),
        serverMod({ path: "/mods/b", name: "B" }),
      ...
      filesByModPath: {
        "/mods/a": [{ relativePath: "db/base.json", sizeBytes: 10, contentHash: "hashA" }],
        "/mods/b": [{ relativePath: "db/base.json", sizeBytes: 12, contentHash: "hashB" }],
      },
    });
    const ow = report.sections.O.filter((f) => f.kind === "file_overwrite");
    expect(ow).toHaveLength(1);
    expect(ow[0].detail?.contentIdentical).toBe(false);
    expect(ow[0].detail?.hashCount).toBe(2);
    expect(report.sections.O.filter((f) => f.kind === "duplicate_file")).toHaveLength(0);
  });

  it("部分文件 hash 未知 -> 按真冲突处理（无法证明相同）", () => {
    const report = run({
      mods: [
        serverMod({ path: "/mods/a", name: "A" }),
        serverMod({ path: "/mods/b", name: "B" }),
      ...
      filesByModPath: {
        "/mods/a": [{ relativePath: "db/base.json", sizeBytes: 10, contentHash: "hashA" }],
        "/mods/b": [{ relativePath: "db/base.json", sizeBytes: 12, contentHash: null }],
      },
    });
    const ow = report.sections.O.filter((f) => f.kind === "file_overwrite");
    expect(ow).toHaveLength(1);
