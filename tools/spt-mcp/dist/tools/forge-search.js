...
export const ForgeSearchInput = z
    ...
    .strict();
export function runForgeSearch(args) {
    const parsed = ForgeSearchInput.safeParse(args);
    if (!parsed.success) {
        return errEnv("spt_forge_search", "无效输入", SPT_ERROR_CODES.INVALID_INPUT, parsed.error.message);
    }
    try {
        ...
        return okEnv("spt_forge_search", `匹配 ${result.total} 个 mod`, {
        ...
        return errEnv("spt_forge_search", `搜索失败：${error instanceof Error ? error.message : String(error)}`, SPT_ERROR_CODES.INTERNAL_ERROR);
    }
}
//# sourceMappingURL=forge-search.js.map