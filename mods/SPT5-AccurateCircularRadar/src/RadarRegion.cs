...
namespace Radar
{
    ...
    /// </summary>
    /// <remarks>
    /// 4.1 -> 5.0 适配（IL2CPP）：<c>new GameObject(name, params Type[])</c> 在 interop 里参数是
    /// <c>Il2CppReferenceArray&lt;Il2CppSystem.Type&gt;</c>，托管 <c>typeof(...)</c> 无法直接传入；
    /// 改为 <c>new GameObject(name)</c> + 逐个 <c>AddComponent&lt;T&gt;()</c>（行为等价）。
    /// </remarks>
    public class RadarRegion
    {
        ...
        public RadarRegion(Vector3[] worldCorners)
        {
            ...
            _root = new GameObject("RadarRegion");
            ...
            var fillObject = new GameObject("RadarRegionFill");
            fillObject.AddComponent<RectTransform>();
            fillObject.AddComponent<CanvasRenderer>();
            fillObject.AddComponent<PolygonGraphic>();
