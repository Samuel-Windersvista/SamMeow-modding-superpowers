...
namespace Radar
{
    ...
    /// <para>
    /// 4.1 -> 5.0 适配（IL2CPP）：<c>Transform.Find</c> 返回的代理按声明类型（Transform）包装，
    /// 直接 <c>(RectTransform)</c> 强转会抛 InvalidCastException；改用 <c>TryCast&lt;RectTransform&gt;()</c>。
    /// </para>
    /// </remarks>
    internal sealed class RadarHudLayout
    {
        ...
        /// </summary>
        public static RectTransform BorderTransform { get; private set; }
        ...
        public RadarHudLayout(Transform hudRoot, GameObject fpsCamera)
        {
            ...
            _baseTransform = hudRoot.Find("Radar").TryCast<RectTransform>();
            _baseObject = _baseTransform.gameObject;
            _initialScale = _baseTransform.localScale;

            BorderTransform = hudRoot.Find("Radar/RadarBorder").TryCast<RectTransform>();
            BorderTransform.SetAsLastSibling();
            _borderImage = BorderTransform.GetComponent<Image>();

            PulseTransform = hudRoot.Find("Radar/RadarPulse").TryCast<RectTransform>();
