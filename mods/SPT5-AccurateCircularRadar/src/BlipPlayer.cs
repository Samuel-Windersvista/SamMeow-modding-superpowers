...
namespace Radar
{
    ...
    /// <remarks>
    /// 4.1 -> 5.0 适配：<c>GetOrAddComponent&lt;T&gt;()</c>（Assembly-CSharp 的扩展方法）在 5.0 interop 下
    /// 不再可靠，改为等价的 <c>GetComponent</c> + <c>AddComponent</c>。
    /// </remarks>
    public class BlipPlayer : Target
    {
                ...
                case EPlayerSide.Savage:
                    switch (_enemyPlayer.Profile.Info.Settings.Role)
                    {
                        ...
                        case WildSpawnType.assaultGroup:
                            ...
                            break;
                        // 5.0/SPT 适配：SPT 的 PMC bot 是 side=Savage + role=pmcBEAR/pmcUSEC
                        // （桥的分类实证），上游按 side 取色的写法会让它们落入 default「boss」分支
                        // 被渲染成红色。这里按 role 给回 PMC 颜色。
        ...
        private void UpdateCorpseOutline(Color factionColor)
        {
            ...
            {
                _outline = blipImage.GetComponent<Outline>();
                if (_outline == null)
                    _outline = blipImage.gameObject.AddComponent<Outline>();
