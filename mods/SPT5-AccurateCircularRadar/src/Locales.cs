        ...
        private static readonly Dictionary<string, Dictionary<string, string>> Translations = new Dictionary<string, Dictionary<string, string>>()
{
            {"EN", new Dictionary<string, string>{
                ...
                {"radar_exfil_enable","Exfiltraction"},
                {"radar_exfil_enable_info","Show the available exfiltractions"},
            ...
            {"RU", new Dictionary<string, string>{
                ...
                {"radar_exfil_enable","Точки выхода"},
                {"radar_exfil_enable_info","Показывать доступные точки выхода"},
            ...
            {"KO", new Dictionary<string, string>{
                ...
                {"radar_exfil_enable","탈출 지점"},
                {"radar_exfil_enable_info","사용 가능한 탈출 지점 표시"},
        ...
        /// <summary>
        /// 归一化语言代码：trim 后必须精确命中词表（EN / ZH / RU / KO），否则回退 <see cref="FallbackLanguage"/>。
        /// </summary>
        /// <remarks>
        /// 配置键名是「翻译后的字符串」，在插件加载时按语言一次性确定（见 <see cref="RadarConfig"/>），
        /// 因此语言解析必须与键集迁移逻辑共用同一套语义，否则迁移会按与绑定不同的语言找旧键。
        /// </remarks>
        public static string ResolveLanguage(string raw)
        {
            string value = (raw ?? string.Empty).Trim();
            return Translations.ContainsKey(value) ? value : FallbackLanguage;
        }

        /// <summary>
        /// 原始值是否是受支持的语言代码（trim 后精确匹配，不做回退）。
        /// 用于标记文件校验：未知值视为「损坏」，迁移直接跳过而不猜测旧键集。
        /// </summary>
        public static bool IsSupportedLanguage(string raw)
        {
            return Translations.ContainsKey((raw ?? string.Empty).Trim());
        }

        /// <summary>
        /// The string for <paramref name="key"/> in <paramref name="language"/>, falling back to English
        /// for an unsupported language or an untranslated key.
        /// </summary>
        public static string GetTranslatedString(string key, string language)
        {
            string resolved = ResolveLanguage(language);

            if (Translations.TryGetValue(resolved, out Dictionary<string, string> strings) &&
        ...
        {
            return GetTranslatedString(key, RadarConfig.Language.Value);
        }
    }
}
