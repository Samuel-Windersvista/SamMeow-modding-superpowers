...
namespace SamMeow.SPT.PerformanceTweaks.Patches
{
    ...
    /// (GameWorld.cs:1155-1167);TryGetObservedPlayer 同理(:1141-1153)。多个系统每帧频繁调用。
    ...
    /// override(ClientNetworkGameWorld.cs:23-31),该 override 最终调用 base(GameWorld).RegisterPlayer,
    /// 故 Postfix 挂 base 上即全覆盖。
    /// 安全阀:索引与权威集合数量不一致(注册/注销滞后一帧)时,从权威集合全量重建再查,保证不错判;
    /// 索引未命中返回 false,与原版全扫找不到的语义一致。
    /// 战局结束 GameWorld.Clear 清空权威集合(GameWorld.cs:2175-2180),静态索引跨战局残留——
    /// 防御:RegisterPlayer 首次调用时若权威集合已空而索引非空则整表重置。
    /// 证据:external/decompile-cache/eft-0.16-spt3114/EFT/GameWorld.cs:552,554,1141-1167,2248-2323
    /// </summary>
    internal static class PlayerIndexPatch
    {
        ...
        /// Player 侧权威 = AllAlivePlayersList(与 allAlivePlayersByID 同步维护,GameWorld.cs:554,2261,2312);
        /// Observed 侧权威 = allObservedPlayersByID(public 字段,GameWorld.cs:562,ClientNetworkGameWorld.cs:27)。
        /// </summary>
