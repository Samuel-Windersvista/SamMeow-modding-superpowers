...
using HarmonyLib;
using SamMeow.SPT.PerformanceTweaks.Patches;

namespace SamMeow.SPT.PerformanceTweaks
{
    /// <summary>
    /// SPT 3.11.x (EFT 0.16) 客户端性能优化 — 14 补丁（P1-P14）。
    /// 设计原则（见 docs/eft-0.16-性能分析与优化mod可行性报告.md §7.4）：
    ///   1. 全部 Prefix/Postfix，禁用 Transpiler（与 spt-singleplayer 的 UpdateManual IL patch 正交共存）
    ///   2. 每补丁独立开关，fail-open（补丁自身异常时放行原逻辑）
    ///   3. SAIN/QB 等兼容性经源码审计，见报告 §7.3 与 KB client-mod-compat-audit-playbook
    ///   4. 不改公开 API、不改数据结构、不序列化状态
    /// </summary>
    [BepInPlugin(ModGuid, ModName, ModVersion)]
    public class PerformanceTweaksPlugin : BaseUnityPlugin
    {
        public const string ModGuid = "com.sammeow.spt311.performancetweaks";
        public const string ModName = "SamMeow PerformanceTweaks";
        ...
        private void Awake()
        {
            ...
            if (SainDetected)
            {
                // 经 SAIN 源码审计（E:\云文件\GitHub\Moew-SAIN-For-3114）：SAIN 不 patch
                // CheckLookEnemy / UpdateLook / CalcPath / AddEnemy 语义（其 AddEnemy patch 仅为
                // null 守卫），与本 mod 全部补丁零目标级冲突，无需禁用，共存互补。
                Log.LogInfo("检测到 SAIN — 经源码审计与本 mod 无补丁目标冲突，全部补丁保持启用（互补关系）");
            }

            _harmony = new Harmony(ModGuid);
            int applied = 0;
            // P1 用低优先级 prefix：QuestingBots 在同方法（CheckLookEnemy）有睡眠 bot 的
            ...
            applied += TryPatch(LocalBotFbbikFrameSkipPatch.TargetMethod(), typeof(LocalBotFbbikFrameSkipPatch), "P13 本地AI的IK隔帧");
            applied += TryPatch(LocalAvoidanceFrameSkipPatch.TargetMethod(), typeof(LocalAvoidanceFrameSkipPatch), "P14 AI扎堆斥力分帧");

            Log.LogInfo($"{ModName} v{ModVersion} 已加载，{applied}/14 个补丁生效");
        }
