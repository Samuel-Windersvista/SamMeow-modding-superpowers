...
{
    /// <summary>
    /// P7: BotsList.UpdateByUnity 远距 bot 分帧降频。
    /// 原版：每帧遍历全部 bot 调 UpdateManual，单 bot 异常记录 _loggedErrors，尾部 AddFromList()
    /// （BotsList.cs:271-288）。大图大量 bot 时每帧全量更新是主线程主要 AI 成本。
    /// 补丁：Prefix 替代实现——距主玩家 &lt;= 近距离阈值（默认 100m）的 bot 每帧更新；
    /// 超出阈值的 bot 按 Time.frameCount % N == bot.Id % N 分帧（N 默认 2，可配 1-8；N=1 不降频）。
    /// 原方法的异常语义（_loggedErrors 去重记录）与尾调用 AddFromList() 完整保留；
    /// 字段名已按 4.1.2 树核实：_botOwners :35、_loggedErrors :41（3.11 的 hashSet_0/hashSet_1 已更名）。
    /// 主玩家位置取 Singleton&lt;GameWorld&gt;.Instance.MainPlayer（public 字段 :541），取不到时
    /// 放行原版完整遍历（fail-open）。整体脚手架包 try/catch，出错放行原版。
    /// 证据：external/decompile-cache/eft-0.16.9.5-spt412/BotsList.cs:271-288
    /// </summary>
    internal static class BotFrameBudgetPatch
    {
        internal static MethodBase TargetMethod()
        {
            return AccessTools.Method(typeof(BotsList), "UpdateByUnity");
        }

        // ReSharper disable once InconsistentNaming
        public static bool Prefix(BotsList __instance)
        {
            ...
            try
            {
                ...
                if (n <= 1 || __instance._botOwners == null || __instance._botOwners.Count == 0)
                {
                ...
                HashSet<int> failed = __instance._loggedErrors;
                foreach (BotOwner item in __instance._botOwners)
                {
