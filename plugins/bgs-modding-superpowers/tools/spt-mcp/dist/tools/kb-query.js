...
export const KbQueryInput = z
    ...
    .strict();
export function runKbQuery(args) {
    const parsed = KbQueryInput.safeParse(args);
    if (!parsed.success) {
        return errEnv("spt_kb_query", "无效输入", SPT_ERROR_CODES.INVALID_INPUT, parsed.error.message);
    }
    const { topic, domain, version, keyword } = parsed.data;
    try {
        const kbRoot = resolveKbRoot();
        const index = loadJson(kbRoot, KB_INDEX_REL);
        ...
        const matches = entries.filter((entry) => {
            if (topicLower) {
                if (entry.topic.toLowerCase() !== topicLower)
                    return false;
            }
            if (domainLower) {
                const d = entry.domain.toLowerCase();
                if (d !== domainLower && d !== "both")
                    return false;
            }
            if (versionTrim) {
                const versions = (Array.isArray(entry.version) ? entry.version : [entry.version]).map((v) => v.toLowerCase());
                const want = versionTrim.toLowerCase();
                if (!versions.includes(want) && !versions.includes("通用"))
                    return false;
            }
            if (keywordLower) {
                const haystack = [
        ...
        return okEnv("spt_kb_query", `知识库匹配 ${matches.length} 条`, {
            matches,
        ...
        return errEnv("spt_kb_query", `查询失败：${error instanceof Error ? error.message : String(error)}`, SPT_ERROR_CODES.INTERNAL_ERROR);
    }
}
