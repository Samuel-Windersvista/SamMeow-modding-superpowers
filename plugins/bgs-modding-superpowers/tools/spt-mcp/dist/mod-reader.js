...
export function readDllMetadata(dllPaths) {
...
// -----------------------------------------------------------------------------
export function readPackageJson(modPath) {
    let text;
    try {
...
export function readServerModMetadataFromPkg(modPath) {
...
export const readServerModMetadata = readServerModMetadataFromPkg;
/** 从 helper 输出构造 ServerModMetadata（DLL 来源，无 package.json） */
export function serverModFromDll(dllMeta, modPath) {
    return {
        type: "server",
        path: modPath,
...
export function readClientModMetadata(dllPath) {
 ...
 */
export function readModMetadata(modPath, type) {
    if (type === "server") {
...
export function findTopLevelDll(modPath) {
    let entries;
    try {
    ...
    for (const entry of entries) {
        if (!entry.isFile())
            continue;
        if (extname(entry.name).toLowerCase() !== ".dll")
            continue;
...
export function findAnyDll(modPath) {
    for (const f of walkFiles(modPath)) {
        if (extname(f).toLowerCase() !== ".dll")
            continue;
        return f;
    }
    return null;
 ...
 */
export function listServerMods(basePath) {
    const out = [];
    let entries;
    try {
 ...
 */
export function listNestedServerMods(modPath) {
    const out = [];
    const userModsDir = join(modPath, "user", "mods");
    if (!isDirectory(userModsDir))
        return out;
    let nestedEntries;
    try {
...
export function listClientMods(basePath) {
    const out = [];
 ...
 */
export function scanModFiles(modPath) {
    const out = [];
    for (const f of walkFiles(modPath)) {
        let size = 0;
        try {
...
export function readConfigKeys(modPath) {
    const out = [];
    for (const f of walkFiles(modPath)) {
        if (extname(f).toLowerCase() !== ".json")
            continue;
        if (basename(f).toLowerCase() === "package.json")
            continue;
        let parsed;
        try {
...
//# sourceMappingURL=mod-reader.js.map