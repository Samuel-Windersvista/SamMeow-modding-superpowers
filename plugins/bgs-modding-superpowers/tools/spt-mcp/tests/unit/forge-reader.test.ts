...
/**
 * 仓库内真实知识库根。
 * 本测试位于 plugins/bgs-modding-superpowers/tools/spt-mcp/tests/unit，
 * 上溯 6 级（unit→tests→spt-mcp→tools→bgs-modding-superpowers→plugins→仓库根）
 * 才是仓库根的 knowledge/spt-kb。
 */
...
describe("forge-reader: 目录加载", () => {
  it("真实 mods-catalog.json 加载全部 mod（含 BOM 兼容）", () => {
    const kbRoot = repoKbRoot();
    const catalog = loadCatalog(kbRoot);
    // 与文件实际条目数对比，而非硬编码历史数量（归档会随新增 mod 变化）
    const raw = JSON.parse(
      readFileSync(resolve(kbRoot, "archive/forge/api/mods-catalog.json"), "utf8").replace(
        /^\uFEFF/,
        "",
      ),
    );
  ...
  it("真实 hot-index.json 加载全部条目（含 BOM 兼容）", () => {
    const kbRoot = repoKbRoot();
    const hot = loadHotIndex(kbRoot);
    const raw = JSON.parse(
      readFileSync(resolve(kbRoot, "archive/forge/hot-index.json"), "utf8").replace(
        /^\uFEFF/,
        "",
      ),
    );
    expect(hot.length).toBe(raw.length);
...
describe("forge-reader: 真实知识库搜索冒烟", () => {
  ...
  it("resetForgeCache 后可重新加载", () => {
    resetForgeCache();
    // 数量随归档变化，断言非空即可（与 mods-catalog 实际条目一致）
    expect(loadCatalog(repoKbRoot()).length).toBeGreaterThan(1000);
    resetForgeCache();
  });
});
