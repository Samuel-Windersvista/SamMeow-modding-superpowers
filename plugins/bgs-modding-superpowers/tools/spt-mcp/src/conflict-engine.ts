...
export interface AnalyzeConflictInput {
  mods: ModMetadata[];
  /** mod.path -> 文件清单（由工具层扫描后传入，保持引擎纯函数） */
  filesByModPath?: Record<string, ModFileEntry[]>;
  /** mod.path -> 配置文件 key 清单（同上） */
  configsByModPath?: Record<string, ConfigFileKeys[]>;
  /** mod.path -> Harmony patch 信息（由工具层通过 il-helper 读取后传入） */
  ilPatches?: ClientPatchInfo[];
  /** 目标 SPT 版本，默认 "4.1" */
  targetSptVersion?: string;
}

// -----------------------------------------------------------------------------
// 语义化版本比较（SPT 用 SemanticVersioning.Range，如 "~4.1.0"）
// -----------------------------------------------------------------------------
...
export function hasExplicitPatch(raw: string): boolean {
  const cleaned = raw.trim().replace(/^[vV]/, "");
...
function exactVersionMatch(raw: string, target: ParsedVersion): boolean {
  const cleaned = raw.trim().replace(/^[vV]/, "");
  if (/^x$/i.test(cleaned)) return true;
  const wildcard = cleaned.match(/^(\d+)[.]x$/i);
  if (wildcard) return target.major === parseInt(wildcard[1], 10);
 ...
 */
export function isCompatibleWithTarget(declared: string, target: string): boolean {
...
export function serverModsOf(mods: ModMetadata[]): ServerModMetadata[] {
  return mods.filter((m): m is ServerModMetadata => m.type === "server");
}

export function predictLoadOrder(mods: ModMetadata[]): LoadOrderEntry[] {
...
function detectGuidDuplicates(mods: ModMetadata[]): ConflictFinding[] {
  const byGuid = new Map<string, ServerModMetadata[]>();
...
function detectVersionMismatch(mods: ModMetadata[], target: string): ConflictFinding[] {
  const findings: ConflictFinding[] = [];
  for (const m of serverModsOf(mods)) {
    const declared = m.sptVersion ?? m.compatibleVersion;
    if (!declared) continue;
    if (isCompatibleWithTarget(declared, target)) continue;
    findings.push({
      severity: "B",
...
function isIgnoredOverwritePath(relPath: string): boolean {
  if (IGNORED_OVERWRITE_PATHS.has(relPath)) return true;
  // 构建产物/源码目录内的重复文件（src/、dist/、obj/、bin/）跳过
  const lower = relPath.toLowerCase();
  if (lower.startsWith("src/") || lower.startsWith("dist/") || lower.startsWith("obj/") || lower.startsWith("bin/")) return true;
  return false;
}

function detectFileOverwrites(
  mods: ModMetadata[],
  filesByModPath?: Record<string, ModFileEntry[]>,
): ConflictFinding[] {
  if (!filesByModPath) return [];
  const byPath = new Map<string, { name: string; modPath: string; sizeBytes: number; hash?: string | null }[]>();
  for (const m of mods) {
    const files = filesByModPath[m.path];
    if (!files) continue;
    for (const f of files) {
      if (isIgnoredOverwritePath(f.relativePath)) continue;
  ...
  for (const [relPath, owners] of byPath) {
    ...
    const hashes = ownerList.map((o) => o.hash).filter((h): h is string => typeof h === "string");
    ...
    findings.push({
      severity: "O",
      kind: "file_overwrite",
      message: `${distinct.size} 个 mod 包含相同文件 "${relPath}" 且内容不同（${
        knownHashes.size > 1 ? `${knownHashes.size} 种内容` : "部分 hash 未知"
      }），MO2 优先级仲裁谁覆盖谁，未胜出的内容被遮蔽`,
      mods: ownerList.map((o) => o.name),
...
function detectConfigCollisions(
  mods: ModMetadata[],
  configsByModPath?: Record<string, ConfigFileKeys[]>,
): ConflictFinding[] {
  if (!configsByModPath) return [];
  const byFile = new Map<string, { name: string; modPath: string; keys: string[] }[]>();
  for (const m of mods) {
...
function intersectAll(keyLists: string[][]): string[] {
  if (keyLists.length === 0) return [];
  const [first, ...rest] = keyLists;
  const seen = new Set(first);
  for (const list of rest) {
    const next = new Set<string>();
    for (const k of list) {
      if (seen.has(k)) next.add(k);
    }
    seen.clear();
 ...
 */
export function detectIlConflicts(patches?: ClientPatchInfo[]): ConflictFinding[] {
  if (!patches || patches.length === 0) return [];

  // 按 (targetType, targetMethod) 分组
  const byTarget = new Map<string, ClientPatchInfo[]>();
...
export function analyzeConflicts(input: AnalyzeConflictInput): ConflictReport {
  const target = input.targetSptVersion ?? DEFAULT_TARGET_SPT_VERSION;

  const sections: Record<SeverityLevel, ConflictFinding[]> = {
    B: [],
    S: [],
    O: [],
    C: [],
