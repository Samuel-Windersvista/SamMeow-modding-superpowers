...
export function inferModType(modPath: string, sptPath?: string): ModType {
  const p = toPosix(modPath).toLowerCase();
  if (p.endsWith(".dll") || p.includes("/bepinex/")) return "client";
  if (sptPath && sptPath.length > 0) {
    const s = toPosix(sptPath).toLowerCase().replace(/\/+$/, "");
    if (p.startsWith(`${s}/`)) {
      const rel = p.slice(s.length + 1);
      if (rel.startsWith("bepinex/") || rel.includes("/bepinex/")) return "client";
      if (rel.startsWith("user/mods/") || rel.includes("/user/mods/")) return "server";
    }
  }
  return "server";
}

export function runAnalyzeConflicts(args: unknown): Envelope {
  ...
  try {
    ...
    const filesByModPath: Record<string, ReturnType<typeof scanModFiles>> = {};
    const configsByModPath: Record<string, ConfigFileKeys[]> = {};
    ...
    return okEnv(
      "spt_analyze_conflicts",
      `分析 ${mods.length} 个 mod（含 ${ilPatches.length} 个 Harmony patch），发现 ${report.summary.total} 个冲突`,
      {
        report,
