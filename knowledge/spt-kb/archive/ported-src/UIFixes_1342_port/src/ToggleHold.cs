...
public class ToggleHoldIdleState(KeyBindingClass keyCombination) : KeyBindingClass.KeyCombinationState(keyCombination)
{
    public override ECommand GetCommand(float deltaTime)
    {
        ...
        HandleKeys(false);
        KeyCombination.ChangeState((KeyBindingClass.EKeyState)ToggleHoldState.ClickOrHold);
    ...
    public override ECommand GetCommand(float deltaTime)
    {
        if (GetKeysStatus(out EKeyPress ekeyPress))
        {
            if (ekeyPress == EKeyPress.Hold)
            {
                ...
                {
                    KeyCombination.ChangeState((KeyBindingClass.EKeyState)ToggleHoldState.Holding);
                }
        ...
        UnhandleKeys(null);
        KeyCombination.ChangeState((KeyBindingClass.EKeyState)ToggleHoldState.Idle);
        return ECommand.None;
    }
...
public class ToggleHoldHoldState(KeyBindingClass keyCombination) : KeyBindingClass.KeyCombinationState(keyCombination)
{
    public override ECommand GetCommand(float deltaTime)
    {
        ...
        return KeyCombination is ToggleKeyCombination toggleKeyCombination ? (ECommand)AccessTools.Field(typeof(ToggleInputKeyCombination), "_disableCommand").GetValue(toggleKeyCombination) : KeyCombination.ecommand_0;
    }
}
