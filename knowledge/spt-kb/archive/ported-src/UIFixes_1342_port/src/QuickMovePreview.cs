...
public class QuickMovePreview : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    private void ShowEquipHighlight()
    {
        ...
        if (_itemContext.Item.Parent.Container is Slot equippedSlot && equippedSlot.ParentItem is InventoryEquipment)
        {
            return;
        }

        var itemAddress = FindSlotToPickUp(equipment, _itemContext.Item);
    ...
    private static ItemAddress FindSlotToPickUp(InventoryEquipment equipment, Item item)
    {
        foreach (EquipmentSlot slotType in Enum.GetValues(typeof(EquipmentSlot)))
        {
            Slot slot = equipment.GetSlot(slotType);
