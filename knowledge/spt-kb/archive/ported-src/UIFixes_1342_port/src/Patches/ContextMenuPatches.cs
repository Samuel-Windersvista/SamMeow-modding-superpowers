...
public static class ContextMenuPatches
{
    ...
    public class CreateSubInteractionsTradingPatch : ModulePatch
    {
        ...
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools.Method(
                typeof(ContextInteractions<EItemInfoButton>),
                nameof(ContextInteractions<>.CreateSubInteractions));
        }
            ...
            ISubInteractions subInteractionsWrapper)
        {
            ...
            if (parentInteraction == EItemInfoButton.Insure)
            {
                ...
                CurrentInsuranceInteractions = new(item, __instance.UIContext, playerRubles);
                CurrentInsuranceInteractions = MultiSelect.Active ?
                    new(MultiSelect.ItemContexts.Select(ic => ic.Item), __instance.UIContext, playerRubles) :
                    new(item, __instance.UIContext, playerRubles);
            ...
            if (parentInteraction == EItemInfoButton.Repair)
            {
                ...
                CurrentRepairInteractions = new(item, __instance.UIContext, playerRubles);
                subInteractionsWrapper.SetSubInteractions(CurrentRepairInteractions);
    ...
    public class EnableInsureInnerItemsPatch : ModulePatch
    {
        ...
        public static bool Prefix(ContextInteractionSwitcherClass __instance, EItemInfoButton button, ref IResult __result)
        {
            ...
            IEnumerable<Item> items = MultiSelect.Active ? MultiSelect.ItemContexts.Select(ic => ic.Item) : [SwitcherItem(__instance)];
            IEnumerable<InsuranceItem> insuranceItemes = items.Select(InsuranceItem.FindOrCreate);
            IEnumerable<InsuranceItem> insurableItems = insuranceItemes.SelectMany(SwitcherInsurance(__instance).GetItemChildren)
                .SelectMany(SwitcherInsurance(__instance).GetItemChildren)
                .Concat(insuranceItemes)
                .Where(i => SwitcherInsurance(__instance).ItemTypeAvailableForInsurance(i) && !SwitcherInsurance(__instance).InsuredItems.Contains(i));
    ...
    public class DisableInsureOnUninsurable : ModulePatch
    {
        ...
        public static void Postfix(ContextInteractionSwitcherClass __instance, EItemInfoButton button, ref bool __result)
        {
            ...
            if (SwitcherInsurance(__instance) == null || !SwitcherInsurance(__instance).ItemTypeAvailableForInsurance(SwitcherItem(__instance)))
            {
                __result = false;
            }
        }
    ...
    public class ModdingItemContextPatch : ModulePatch
    {
        ...
        [PatchPrefix]
        public static bool Prefix(ModdingItemContext __instance, Action closeAction, ref ContextInteractions<EItemInfoButton> __result)
        {
            ...
            : ModdingItemInteractions(itemContext, itemUiContext, closeAction)
        {
            ...
            public override void CreateSubInteractions(EItemInfoButton parentInteraction, ISubInteractions subInteractionsWrapper)
            {
                ...
                {
                    subInteractionsWrapper.SetSubInteractions(new WishListInteractions(ItemContext, UIContext));
                }
                else
                {
    ...
    private static Item SwitcherItem(ItemContextInteractionsSwitcher context) => (Item)AccessTools.Field(typeof(ItemContextInteractionsSwitcher), "_item").GetValue(context);

    private static InsuranceCompany SwitcherInsurance(ItemContextInteractionsSwitcher context) => (InsuranceCompany)AccessTools.Field(typeof(ItemContextInteractionsSwitcher), "_insurance").GetValue(context);
}