...
public static class DropdownPatches
{
    ...
    public static void Enable()
    {
        ...
        // 4.1.2: EmptySlotRemovePointerDownPatch removed (EmptyItemView.OnPointerDown is an explicit interface impl)
    ...
    public class ModdingScreenListenClosePatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools.Method(typeof(ModdingScreenSlotView), nameof(ModdingScreenSlotView.DropDownButtonClickHandler));
        }
    ...
    public class FixBlockedItemSwapPatch : ModulePatch
    {
        ...
        public static bool Prefix(BuildItemSelector __instance, Item item, Slot slot, bool simulate, ref Error error, ref bool __result)
        {
            ...
            {
                removeOperation = InteractionsHandlerClass.Remove(slot.ContainedItem, __instance.InventoryController, item == null && simulate);
            ...
            OperationResult<AddOperation> addOperation = default;
            if (item != null)
            {
