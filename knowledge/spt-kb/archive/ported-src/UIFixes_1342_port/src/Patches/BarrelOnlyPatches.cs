        ...
        public class BarrelLoadAmmoInteractions : ContextInteractions<BarrelLoadAmmoInteractions.EMagInteraction>
        {
            ...
            public BarrelLoadAmmoInteractions(Weapon weapon, ItemUiContext itemUiContext) : base(itemUiContext)
            {
                ...
                foreach (var (ammoTemplateId, count) in FindCompatibleAmmo())
                {
                    ...
                    string text = string.Format("<b><color=#C6C4B2>{0}</color> <color=#ADB8BC>({1})</color></b>", nameKey.Localized(EFT.EStringCase.None), count);

                    base.method_2(ammoTemplateId, text, () => LoadAmmo(ammoTemplateId), null);
                }
            }
            ...
            private Dictionary<string, int> FindCompatibleAmmo()
            {
                ...
                {
                    if (CheckCompatability(ammoItem))
                    {
            ...
            private async Task<bool> TryLoadFromContainer(CompoundItem container, string ammoTemplateId)
            {
                ...
                var matchingAmmo = ammo.Where(a => a.TemplateId == ammoTemplateId)
