...
public static class TraderAvatarPatches
{
    ...
    public class ShowIconsPatch : ModulePatch
    {
            ...
            GameObject ____availableToStartQuestsIcon)
        {
            ...
            var traderQuests = quests.GetQuests(___traderInfo_0.Id);
            ...
            bool handInsAvailable = Settings.HandOverQuestItemsIcon.Value && QuestHandInAvailable(traderQuests, ___abstractQuestControllerClass.InventoryController.Inventory, ___abstractQuestControllerClass);
    ...
    public class QuestListItemPatch : ModulePatch
    {
        ...
        public static void Postfix(QuestListItem __instance, Image ____lockedIcon)
        {
            ...
            if (Settings.QuestHandOverQuestItemsIcon.Value &&
                ...
                QuestHandInAvailable([__instance.Quest], inventory, null))
            {
    ...
    public class HandoverPatch : ModulePatch
    {
        ...
        public static async void Postfix(QuestClass quest, Task<IResult> __result)
        {
            ...
            if (result.Succeed)
            {
                quest.SetStatus(quest.QuestStatus, false, false);
            }
        }
    }

    private static bool QuestHandInAvailable(IEnumerable<QuestClass> quests, Inventory inventory, EFT.Quests.QuestController questController)
    {
        ...
        foreach (var quest in inProgressQuests)
        {
            if (quest.Conditions.TryGetValue(EQuestStatus.AvailableForFinish, out var finishConditions))
            {
                ...
                if (conditions.Any(c => ConditionHandInAvailable(c, inventory, questController)))
                {
                    return true;
                }
            }
    ...
    private static bool ConditionHandInAvailable(Condition condition, Inventory inventory, EFT.Quests.QuestController questController)
    {
        if (condition is ConditionHandoverItem conditionHandoverItem)
        {
            ...
            {
                return EFT.Quests.ConditionalController<EFT.Quests.Quest>.GetItemsForCondition(inventory, conditionHandoverItem).Any();
            }
        }
