...
public static class MagPresetsPatches
{
    ...
    public class LoadMagPresetsPatch : ModulePatch
    {
        ...
        {
            return AccessTools.Method(typeof(MagazineBuildPresetClass), nameof(MagazineBuildPresetClass.GetUserContainers));
        }
    ...
    public class RequirePresetNamePatch : ModulePatch
    {
        ...
        public static void Postfix(MagPresetEditor ____presetEditor, DefaultUiButtonNewStyle ____saveButton)
        {
            ...
            if (string.IsNullOrEmpty(name))
            {
                ...
                string nameRequiredError = "<color=red>" + "MagPreset/SetNameWindowPlaceholder".Localized(EFT.EStringCase.None) + "</color>";
                int index = tooltip.IndexOf("MagPreset/Tooltip/HasNoChanges".Localized(EFT.EStringCase.None));
                if (index >= 0)
                {
