...
public static class FleaPrevSearchPatches
{
    ...
    public class PreviousFilterButton : MonoBehaviour
    {
        ...
        public void Show(RagfairScreen ragfairScreen, RagFairClass ragfair)
        {
            ...
            _button.SetRawText("< " + "back".Localized(EFT.EStringCase.None), 20);
            ...
            if (!History.Any())
            {
                ragfair.HandleDefaultFilter(EViewListType.AllOffers);
                History.Push(new HistoryEntry() { FilterRule = ragfair.FilterRule }); // Player's saved default rule
            }
        ...
        public void OnOffersLoaded(OfferViewList offerViewList)
        {
            if (!String.IsNullOrEmpty(DelayedHandbookId))
            {
                ...
                // 4.1.2: delayed handbook refresh handled by OfferViewList.Show internally
                return;
            }
        ...
        private void ApplyFullFilter(FilterRule filterRule)
        {
            ...
            // 4.1.2: AddSearchesInRule/method_24 replaced; reuse the existing rule (FilterRule is a struct)

            // These properties don't consistute a new search, so much as a different view of the same search
            // view/sort properties carried over from the original rule
            ...
            _ragfair.SetFilterRule(filterRule, true, true, RagFair.ESetFilterSource.IdSelected);
        }
        ...
        private static void UpdateColumnHeaders(FiltersPanel filtersPanel, ESortType sortType, bool sortDirection)
        {
            ...
            filtersPanel.OnButtonClick(button, sortType);
        }
    }
    ...
    public class OfferViewListCategoryPickedPatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools.Method(typeof(OfferViewList), nameof(OfferViewList.ResetCategorySelection));
        }
    ...
    public class OfferViewListDoneLoadingPatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools.Method(typeof(OfferViewList), nameof(OfferViewList.Show));
        }
