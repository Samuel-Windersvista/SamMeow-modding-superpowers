...
public static class SwapPatches
{
    ...
    public class ToggleForceSwapPatch : ModulePatch
    {
            ...
            ItemUiContext ___itemUiContext_0)
        {
            ...
            {
                // 4.1.2: rerun-highlight helper removed; HighlightItemViewPosition below covers the highlight
    ...
    private static void UpdateSwapHighlight(DraggedItemView draggedItemView, IContainer containerUnderCursor, ItemContextAbstractClass itemUnderCursor)
    {
        ...
        if (operation.Succeeded && operation.Value is SwapOperation swapOperation)
        {
            ...
            ((LocationInGrid)AccessTools.Field(typeof(Grid.PlaceTakenByAnotherItemError), "_location").GetValue(error)).x == gridItemAddress.LocationInGrid.x &&
            ((LocationInGrid)AccessTools.Field(typeof(Grid.PlaceTakenByAnotherItemError), "_location").GetValue(error)).y == gridItemAddress.LocationInGrid.y &&
            (Grid)AccessTools.Field(typeof(Grid.PlaceTakenByAnotherItemError), "_grid").GetValue(error) == gridItemAddress.Grid)
        {
            // show red highlight
            color = R.GridView.InvalidOperationColor;
            targetItem = (Item)AccessTools.Field(typeof(Grid.PlaceTakenByAnotherItemError), "_item").GetValue(error);
        }
        else
        {
            return;
    ...
    public class DetectGridHighlightPrecheckPatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools.Method(typeof(GridItemView), nameof(GridItemView.CheckAcceptHandler));
        }
    ...
    public class FixNoGridErrorPatch : ModulePatch
    {
        ...
        [PatchPostfix]
        public static void Postfix(IEnumerable<EFT.InventoryLogic.IContainer> containersToPut, ref OperationResult<IItemResult> __result, Error ___noSpaceError, Error ___noActionsError)
        {
        ...
        public static void Postfix(DraggedItemView __instance, ItemContextAbstractClass itemUnderCursor)
        {
            ...
            if (SourceContainer is Component sourceComponent)
            {
                ...
                if (panel != null)
                {
                    ...
                    panel.OnCompareRequired(slot, itemUnderCursorContext);
                }
            }
        }
    }
