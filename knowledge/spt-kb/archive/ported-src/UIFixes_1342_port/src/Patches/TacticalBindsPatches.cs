...
public static class TacticalBindsPatches
{
    ...
    public class UseTacticalPatch : ModulePatch
    {
        ...
        private static void ToggleLight(Player player, Item boundItem, LightComponent lightComponent)
        {
            ...
            {
                firearmController.SetLightsState([lightState], false, false);
            }
    ...
    public class InitQuickBindsPatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools.Method(typeof(MainMenuControllerClass), nameof(MainMenuControllerClass.ShowMenuScreen), System.Type.EmptyTypes);
        }
        ...
        public static async void Postfix(MainMenuControllerClass __instance, Task __result)
        {
            ...
            Singleton<SharedGameSettingsClass>.Instance.Control.Controller.UpdateBindings();
        }
    }
    ...
    public class BindTacticalPatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools.Method(typeof(ItemUiContext), nameof(ItemUiContext.BindItem));
        }

        [PatchPostfix]
        public static void Postfix(Item item, EBoundItem index)
        {
            // Don't run on fika remote
            if ((item.Owner as InventoryController)?.IsObserved() == true)
            {
                return;
            }
    ...
    public class UnbindTacticalPatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools.Method(typeof(InventoryController), nameof(InventoryController.UnbindItem));
        }

        [PatchPostfix]
        public static void Postfix(InventoryController controller, EBoundItem boundItemIndex)
        {
            // Don't run on fika remote
            ...
            UpdateQuickbindType(null, boundItemIndex);
