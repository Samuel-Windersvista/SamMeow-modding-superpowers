    ...
    public class AutoOpenPatch : ModulePatch
    {
        ...
        public static bool Prefix(ItemUiContext __instance, ref ItemOperation __result)
        {
            ...
            if (sortingTable != null && !sortingTable.IsNotEmpty)
            {
                ...
                {
                    Singleton<CommonUI>.Instance.InventoryScreen.R().SimpleStashPanel.InitSortingTable(EFT.UI.ItemsPanel.EItemsTab.Gear);
                }
                else if (__instance.ContextType == EItemUiContextType.ScavengerInventoryScreen)
                {
                    Singleton<CommonUI>.Instance.ScavengerInventoryScreen.R().SimpleStashPanel.InitSortingTable(EFT.UI.ItemsPanel.EItemsTab.Gear);
                }
            }
