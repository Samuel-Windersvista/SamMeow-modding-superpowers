...
public class RepairInteractions : ContextInteractions<RepairInteractions.ERepairers>
{
    ...
    private void Load()
    {
        foreach (IRepairer repairer in _repairers)
        {
            ...
            int dupeCount = 1;
            while (CustomNames.ContainsValue(dedupedText))
            {
                dupeCount++;
    ...
    private async void Repair(string repairerId)
    {
        ...
        foreach (var repairStrategy in _repairStrategies)
        {
            ...
            {
                anySuccess = true;
                NotificationManagerClass.DisplayMessageNotification(string.Format("{0} {1:F1}", "Item successfully repaired to".Localized(EFT.EStringCase.None), repairStrategy.Durability()), ENotificationDurationType.Default, ENotificationIconType.Default, null);
            }
        }
