...
) : IDisposable
{
    ...
    private readonly CancellationTokenSource _raidCancellationTokenSource = new();
    ...
    public async UniTask StartAsync(Task loadScreenTask)
    {
        try
        {
            ...
            CancellationToken cancellationToken = _raidCancellationTokenSource.Token;
    ...
    {
        _raidCancellationTokenSource.Cancel();
