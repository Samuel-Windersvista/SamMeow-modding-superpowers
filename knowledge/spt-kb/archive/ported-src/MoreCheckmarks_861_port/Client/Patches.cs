...
namespace MoreCheckmarks
{
    ...
    class QuestItemViewPanelShowPatch
    {
            ...
            TextMeshProUGUI ____questItemLabel)
        {
            try
            {
                ...
                if (profile != null)
                {
                    counts = MoreCheckmarksMod.GetItemCounts(profile, item.StringTemplateId);
                }
                else
                {
                ...
                var neededStruct = MoreCheckmarksMod.GetNeeded(item.StringTemplateId, ref areaNames);
                DataLoader.questDataStartByItemTemplateID.TryGetValue(item.StringTemplateId,
                    out var startQuests);
                DataLoader.questDataCompleteByItemTemplateID.TryGetValue(item.StringTemplateId,
                    out var completeQuests);

                var wishlist = ItemUiContext.Instance.WishlistManager.IsInWishlist(item.StringTemplateId, true, out _);

                var craftTooltip = "";
                var craftRequired = MoreCheckmarksConfig.showCraft &&
                                     MoreCheckmarksMod.GetNeededCraft(item.StringTemplateId, ref craftTooltip);
                ...
                var bartersByTrader = MoreCheckmarksMod.GetBarters(item.StringTemplateId);
            ...
            bool craftRequired, string craftTooltip)
        {
            try
            {
                ...
                if (MoreCheckmarksConfig.showQuestCheckmarks && (item.MarkedAsSpawnedInSession || MoreCheckmarksConfig.showQuestCheckmarksNonFIR))
                {
                    ...
                    {
                        QuestTemplate RawQuestClass = null;
                        ...
                        foreach (QuestDataClass questDataClass in profile.QuestsData)
                        {
                            ...
                            {
                                // UPDATE: Look for the type used in QuestDataClass's Template var of type QuestTemplate with ConditionsDict, for the value
                                foreach (KeyValuePair<EQuestStatus, ConditionCollection> kvp in questDataClass.Template.Conditions)
                                {
                                    kvp.Deconstruct(out EQuestStatus equestStatus, out ConditionCollection gclass);
    ...
    class ItemSpecificationPanelShowPatch
    {
        ...
        static void Postfix(ref QuestItemViewPanel ____questItemViewPanel)
        {
            try
            {
    ...
    class AvailableActionsPatch
    {
        ...
        [HarmonyPatch(typeof(InteractionContextHelper), "GetAvailableActions",
            ...
            try
            {
                foreach (InteractionAction action in __result.Actions)
                {
                    if (action.Name.Equals("Take"))
                    {
                        ...
                        NeededStruct neededStruct = MoreCheckmarksMod.GetNeeded(lootItem.Item.StringTemplateId, ref nullAreaNames);
                        string craftTooltip = "";
                        bool craftRequired =
                            MoreCheckmarksMod.GetNeededCraft(lootItem.Item.StringTemplateId, ref craftTooltip, false);
                        bool wishlist =
                            ItemUiContext.Instance.WishlistManager.IsInWishlist(lootItem.Item.StringTemplateId, true,
                                out EWishlistGroup group);
                        DataLoader.questDataStartByItemTemplateID.TryGetValue(lootItem.Item.StringTemplateId,
                            out QuestPair startQuests);
                        DataLoader.questDataCompleteByItemTemplateID.TryGetValue(lootItem.Item.StringTemplateId,
                        ...
                        List<List<KeyValuePair<string, int>>> bartersByTrader =
                            MoreCheckmarksMod.GetBarters(lootItem.Item.StringTemplateId);
    ...
    class QuestClassStatusPatch
    {
        ...
        {
            preStatus = __instance.questDataClass.Status;
        }
        ...
        static void Postfix(Quest __instance)
        {
            ...
            MoreCheckmarksMod.LogDebug("Quest " + __instance.Template.Name + " queststatus set to " +
                                      __instance.questDataClass.Status);

            try
            {
                if (__instance.questDataClass.Status != preStatus)
                {
                    switch (__instance.questDataClass.Status)
                    {
