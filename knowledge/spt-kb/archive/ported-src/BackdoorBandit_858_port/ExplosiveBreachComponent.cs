...
namespace BackdoorBandit
{
    internal class ExplosiveBreachComponent : MonoBehaviour
    {
        ...
        private static void RemoveItemFromPlayerInventory(Player player)
        {
            ...
            var itemController = (ItemController)foundItem.Parent.GetOwner();
            var discardResult = ItemManipulator.Discard(foundItem, itemController, false, false);
            ...
            discardResult.Value.RaiseEvents(itemController, CommandStatus.Begin);
            discardResult.Value.RaiseEvents(itemController, CommandStatus.Succeed);
        }
