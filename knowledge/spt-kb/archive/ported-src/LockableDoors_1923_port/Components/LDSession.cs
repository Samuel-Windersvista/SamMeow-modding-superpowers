...
namespace LockableDoors.Components
{
    internal class LDSession : MonoBehaviour
    {
        ...
        public static LDSession Instance
        {
            get
            {
                if (_instance == null)
                {
                    ...
                    _instance = Singleton<GameWorld>.Instance.AllPlayersEverExisted.FirstOrDefault(player => player.IsYourPlayer).gameObject.GetOrAddComponent<LDSession>();
                }
                return _instance;
            }
        }
        ...
        private void Awake()
        {
            ...
            Player = GameWorld.AllPlayersEverExisted.FirstOrDefault(player => player.IsYourPlayer);
