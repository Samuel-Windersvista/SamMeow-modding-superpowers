...
namespace acidphantasm_accessibilityindicators.Patches
{
    internal class PlayerDefaultPlayPatch : ModulePatch
    {
        ...
        static void PatchPostfix(Player __instance, EAudioMovementState movementState)
        {
            if (__instance == null 
                ...
                || !Indicators.enable
                || (!__instance.IsAI && ModUtils.IsGroupedWithMainPlayer(__instance) && !Indicators.showTeammates)) return;

            Vector3 position = __instance.Position;
            float distance = (float)distanceInfo.GetValue(__instance);
            bool isTeammate = ModUtils.IsGroupedWithMainPlayer(__instance);
    ...
    internal class PlayerMethod61Patch : ModulePatch
    {
        ...
        {
            distanceInfo = AccessTools.Field(typeof(Player), "_distance");
            return AccessTools.Method(typeof(Player), nameof(Player.StateChangedHandler));
        }

        [PatchPostfix]
        static void PatchPostfix(Player __instance, EPlayerState nextstate)
        {
            if (__instance == null
                || __instance.IsYourPlayer
                || !Indicators.enable
            ...
            if (nextstate is EPlayerState.Sprint)
            {
                Vector3 position = __instance.Position;
                float distance = (float)distanceInfo.GetValue(__instance);
