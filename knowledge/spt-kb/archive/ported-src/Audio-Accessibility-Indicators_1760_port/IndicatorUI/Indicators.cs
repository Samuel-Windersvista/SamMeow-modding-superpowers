...
namespace acidphantasm_accessibilityindicators.IndicatorUI
{
    internal class Indicators : MonoBehaviour
    {
        ...
        {
            var player = ModUtils.GetMainPlayer();
            Transform camera = player.CameraPosition;
            Vector3 cameraPosition = camera.position;

            float voiceDistance = ModUtils.GetDistance(cameraPosition, voicePosition);
            if (voiceDistance > maxVoiceDistance) return;

            Vector3 voiceDirection = voicePosition - cameraPosition;

            float voiceAngle = ModUtils.GetAngle(voiceDirection);
            float playerAngle = ModUtils.GetLookAngle(camera.eulerAngles);
        ...
        public static void PrepareShot(Vector3 shotPosition, string id, bool isTeammate)
        {
            ...
            var shotDistance = ModUtils.GetDistance(cameraPosition, shotPosition);
            if (shotDistance > maxShotDistance) return; 

            var shotDirection = shotPosition - cameraPosition;

            var shotAngle = ModUtils.GetAngle(shotDirection);
            var playerAngle = ModUtils.GetLookAngle(camera.eulerAngles);
            var realShotAngle = shotAngle + playerAngle;

            VerticalityValues value = ModUtils.AboveOrBelowCheck(cameraPosition.y, shotPosition.y);
        ...
        public static void PrepareStep(EAudioMovementState movementState, Vector3 stepPosition, float distance, string id, bool isTeammate)
        {
            ...
            var stepAngle = ModUtils.GetAngle(stepDirection);
            var playerAngle = ModUtils.GetLookAngle(camera.eulerAngles);

            var realStepAngle = stepAngle + playerAngle;

            VerticalityValues value = ModUtils.AboveOrBelowCheck(cameraPosition.y, stepPosition.y);
        ...
        private static void DrawVoiceIndicator(float voiceAngle, float voiceDistance, string accountID, bool isTeammate)
        {
            ...
            float size = ModUtils.CustomInverseLerp(newMinDistance, newMaxDistance, scaleVoiceMax, scaleVoiceMin, voiceDistance);
            float alpha = ModUtils.CustomInverseLerp(newMinDistance, newMaxDistance, 1f, 0.1f, voiceDistance);
        ...
        private static void DrawShotIndicator(float shotAngle, float shotDistance, VerticalityValues value, string accountID, bool isTeammate)
        {
            ...
            float size = ModUtils.CustomInverseLerp(newMinDistance, newMaxDistance, scaleShotMax, scaleShotMin, shotDistance);
            float alpha = ModUtils.CustomInverseLerp(newMinDistance, newMaxDistance, 1f, 0.1f, shotDistance);
        ...
        private static void DrawStepIndicator(float stepAngle, float stepDistance, VerticalityValues value, EAudioMovementState movementState, string accountID, bool isTeammate)
        {
            ...
            float size = ModUtils.CustomInverseLerp(newMinDistance, newMaxDistance, scaleStepMax, scaleStepMin, stepDistance);
            float alpha = ModUtils.CustomInverseLerp(newMinDistance, newMaxDistance, 1f, 0.1f, stepDistance);
