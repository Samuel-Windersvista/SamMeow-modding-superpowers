...
namespace Gaylatea
{
    namespace UseLooseLoot
    {
        ...
        class MakeFoodMedsUsablePatch : ModulePatch
        {
            ...
            {
                if(!(rootItem is EFT.InventoryLogic.Meds) && !(rootItem is EFT.InventoryLogic.FoodDrink)) {
                    return;
                }
        ...
        class FoodMedUser {
            ...
            public void UseAll() {
                // PORT-NOTE: 4.1 中 GamePlayerOwner.Player 重命名为静态 MyPlayer
                GamePlayerOwner.MyPlayer.HealthController.ApplyItem(this.item, EBodyPart.Common);
            }
        }
    }
}