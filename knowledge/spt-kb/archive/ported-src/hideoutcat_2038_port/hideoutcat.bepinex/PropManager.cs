...
namespace hideoutcat.bepinex
{
    internal static class PropManager
    {
        ...
        {
            // 3.11: Singleton<HideoutClass>.Instance.AreaDatas -> 4.1: HideoutRepresentation.AreaDatas
            AreaData kitchenArea = CatDependencyProviders.Hideout?.AreaDatas.FirstOrDefault(x => x.Template.Type == EFT.EAreaType.Kitchen);
            if (kitchenArea != null)
            {
                if (herring == null)
                {
                    AssetBundle propsBundle = AssetBundleLoader.LoadAssetBundle("hideoutcat_props");
                    if (propsBundle == null)
                    {
                        Plugin.Log.LogWarning("hideoutcat_props bundle could not be loaded (AssetBundleLoader plugin missing)");
                        return;
                    }

                    herring = GameObject.Instantiate(propsBundle.LoadAsset<GameObject>("herring_opened"));
        ...
        {
            AreaData heatingArea = CatDependencyProviders.Hideout?.AreaDatas.FirstOrDefault(x => x.Template.Type == EFT.EAreaType.Heating);
            if (heatingArea != null)
            {
