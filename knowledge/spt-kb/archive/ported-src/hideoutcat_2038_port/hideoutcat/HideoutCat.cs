...
namespace hideoutcat
{
    public class HideoutCat : InteractableObject
    {
        ...
        {
            // 3.11: CameraClass.Instance.Flashlight -> 4.1: EFT.CameraControl.CameraManager.Instance.Flashlight
            HideoutCameraFlashlight flashlight = CameraManager.Instance?.Flashlight;
            if (flashlight == null)
                return false;
            return flashlight.IsActive;
        }

        bool IsPlayerInTheWay()
        {
            // 3.11: Singleton<GameWorld>.Instance.MainPlayer -> 4.1: AllPlayersEverExisted.FirstOrDefault(p => p.IsYourPlayer)
        ...
        void GoToRandomArea()
        {
            ...
            // 3.11: Singleton<HideoutClass>.Instance.AreaDatas -> 4.1: CatDependencyProviders.Hideout (HideoutRepresentation) captured by PatchHideoutInit
            var areas = CatDependencyProviders.Hideout?.AreaDatas.OrderBy(x => Random.value);
            if (areas == null)
                return;
