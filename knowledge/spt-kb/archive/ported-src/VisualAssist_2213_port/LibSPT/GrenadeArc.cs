...
public class GrenadeArc : MonoBehaviour
{
    ...
    private static readonly FieldInfo GrenadePrefabField = AccessTools.Field(typeof(Player.BaseGrenadeHandsController), "GrenadePrefab");
    ...
    public void LateUpdate()
    {
        ...
        // HighThrowOperation is high throw, LowThrowOperation is low throw
    ...
    )
    {
        ...
        while (currentDistance <= maxDistance && currentDistance < maxReachableDistance * 0.999f && i < positions.Length)
        {
            ...
            if (Physics.SphereCast(
                    prevPos, 0.05f, arcLine.normalized, out var hit, arcLine.magnitude, LayersMaskController.HitColliderMask.value
                ))
            {
