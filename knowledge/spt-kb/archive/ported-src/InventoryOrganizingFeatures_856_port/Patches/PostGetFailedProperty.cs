            ...
            try
            {
                ...
                if (callerMethod.Name.Equals("OnClick") && callerMethod.ReflectedType == typeof(ItemView))
                {
                    ...
                    if (item == null)
                    {
                        ...
                        NotificationManager.DisplayWarningNotification($"Seion.Iof Error | {__instance.GetType()} Item is still null. You should probably send your bepinex logs to mod developer.");
                        return;
                    }; // null safety
