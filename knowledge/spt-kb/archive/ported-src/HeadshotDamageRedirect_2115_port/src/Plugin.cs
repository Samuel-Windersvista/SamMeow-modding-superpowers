...
{
    [BepInPlugin("com.perfk.dmgRedirect", "Headshot Damage Redirection", "1.7.0")]
    ...
    internal class ApplyDamageInfoPatch : ModulePatch
    {
        ...
        public static void PatchPrefix(ref EBodyPart bodyPartType, ref DamageInfo damageInfo, ref Player __instance)
        {
            ...
            if ((bodyPartType == EBodyPart.Head && Settings.ModEnabled.Value == true) || (bodyPartType == EBodyPart.Chest && Settings.ChestEnabled.Value == true))
            {
                ...
                if (Settings.DebugEnabled.Value)
                    ...
                    {
                        Plugin.LogSource.LogInfo($"Before {bodyPart}  = {__instance.ActiveHealthController.GetBodyPartHealth(bodyPart, false).Current} hp");
                    }
                ...
                if (Settings.DebugEnabled.Value)
                    foreach (EBodyPart bodyPart in Enum.GetValues(typeof(EBodyPart)))
                    {
                        if (bodyPart == EBodyPart.Head)
                            Plugin.LogSource.LogInfo($"After {bodyPart}  = {__instance.ActiveHealthController.GetBodyPartHealth(bodyPart, false).Current - newPartDamage} hp");
                        else if (bodyPart == EBodyPart.Chest && Settings.ChestEnabled.Value == true)
                            Plugin.LogSource.LogInfo($"After {bodyPart}  = {__instance.ActiveHealthController.GetBodyPartHealth(bodyPart, false).Current - newPartDamage} hp");
                        else
                            Plugin.LogSource.LogInfo($"After {bodyPart}  = {__instance.ActiveHealthController.GetBodyPartHealth(bodyPart, false).Current} hp");
                    }
        ...
        private static void createDamageToEachPart(List<EBodyPart> parts, float totalDamage, DamageInfo damageInfo, Player player, StringBuilder stringBuilder)
        {
            ...
            {
                DamageInfo redirectedDamageInfo = CloneDamageInfo(damageInfo, perPart);
        ...
        private static DamageInfo CloneDamageInfo(DamageInfo oldDamageInfo, float newDamage)
        {
