        ...
        public static void PatchPostfix()
        {
            ...
            if (_stash == null)
            {
                foreach (var player in Singleton<GameWorld>.Instance.RegisteredPlayers)
                {
                    ...
                    stash.Grids = new Grid[] { stashGridClass };

                    var traderController = new ItemController(stash, LootRadiusStashGrid.GRIDID, "Nearby Items", false, EOwnerType.Profile);
                    Singleton<GameWorld>.Instance.ItemOwners.Add(traderController, default(GameWorld.ItemOwnerWorldData));
