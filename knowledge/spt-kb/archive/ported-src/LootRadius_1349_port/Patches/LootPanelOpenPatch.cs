...
namespace DrakiaXYZ.LootRadius.Patches
{
    public class LootPanelOpenPatch : ModulePatch
    {
        ...
        )
        {
            ...
            Vector3 playerPosition = Singleton<GameWorld>.Instance.AllPlayersEverExisted.FirstOrDefault(x => x.IsYourPlayer).Position;
            ...
            ____simpleStashPanel.Show(_stash, inventoryController, sourceContext.CreateChild(_stash), true, null, SimpleStashPanel.EStashSearchAvailability.Unavailable, inventoryController, currentTab);
        ...
        private static bool IsLineOfSight(Vector3 endPos)
        {
            ...
            Vector3 startPos = Singleton<GameWorld>.Instance.AllPlayersEverExisted.FirstOrDefault(x => x.IsYourPlayer).MainParts[BodyPartType.head].Position;
