...
namespace FOVFix
{
    ...
    public class CameraUpdatePatch : ModulePatch
    {
        ...
        [PatchPostfix]
        private static void PatchPostfix(OpticComponentUpdater __instance, Camera ___opticCamera, ScopeZoomHandler ___scopeZoomHandler)
        {
            if (___opticCamera != null && ___scopeZoomHandler != null) ___opticCamera.fieldOfView = ___scopeZoomHandler.FiledOfView;
            Logger.LogWarning($"cam is null {(___opticCamera == null)}, scopezoom is null {(___scopeZoomHandler == null)}");
        }
    }
    ...
    public class ScopeZoomPatch : ModulePatch
    {
        ...
        [PatchPostfix]
        private static void Patch(ScopeZoomHandler __instance, SightComponent ___sightComponent, ref float ___float_2)
        {
            /*
            ...
            */
            ...
            //Logger.LogWarning($"possible scope zoom: {Plugin.FovController.CurrentScopeFOV} , real scope zoom: {___sightComponent.ScopeZoomValue}");
        }
    }
    ...
    public class FovRangePatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools.Method(typeof(GameSettingsTab), "Show", new[] { typeof(GameSettingsGroup), typeof(IEftSession), typeof(bool) });
        }

        [PatchPostfix]
        private static void PostFix(NumberSlider ____fov, GameSettingsGroup gameSettings)
        {
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public class LerpCameraPatch : ModulePatch
    {
        ...
        protected override MethodBase GetTargetMethod()
        {
            _playerField = AccessTools.Field(typeof(Player.ItemHandsController), "_player");
        ...
        // Realism Mod compatibility was stripped for the SPT 4.1.2 port (RealismMod.dll unavailable in Refs-410).
            ...
            Transform ____bone0, Transform ____bone1)
        {
            ...
            {
                // Realism Mod compatibility was stripped for the SPT 4.1.2 port (RealismMod.dll unavailable in Refs-410).
                bool smoothPatrolStanceADS = false;
                bool isColliding = false;
                bool isMachinePistol = false;
                bool isPistol = isMachinePistol || Plugin.FovController.IsPistol;
                bool treatAsPistol = isPistol;
                bool isAltPistol = false;
                bool isOptic = __instance.CurrentScope.IsOptic;
                float collsionCameraSpeed = 1f;
                bool isRealLeftShoulder = false;
                bool isDoingLeftShoulder = isRealLeftShoulder || __instance.InLeftStance;
                bool canMoveGunToCamera = false;
                ...
                float headBob = __instance.HeadBobbing;
                ...
                // 3.11 method_19(dt) had no 4.1 equivalent (obfuscated name removed); camera transforms above are fully authored by this prefix.
    ...
    public class PwaWeaponParamsPatch : ModulePatch
    {
        ...
        protected override MethodBase GetTargetMethod()
        {
            playerField = AccessTools.Field(typeof(Player.ItemHandsController), "_player");
            fcField = AccessTools.Field(typeof(ProceduralWeaponAnimation), "_firearmController");
            // 3.11 method_23 (weapon params refresh) -> 4.1 UpdateWeaponVariables()
