...
internal class TarkovVersion : Attribute
{
	...
	public static int BuildVersion => (Assembly.GetExecutingAssembly().GetCustomAttributes(typeof(TarkovVersion), inherit: false)?.Cast<TarkovVersion>()?.FirstOrDefault()?.version).GetValueOrDefault();
	...
	public static bool CheckEftVersion(ManualLogSource Logger, PluginInfo Info, ConfigFile Config = null)
	{
		...
		int filePrivatePart = FileVersionInfo.GetVersionInfo(BepInEx.Paths.ExecutablePath).FilePrivatePart;
