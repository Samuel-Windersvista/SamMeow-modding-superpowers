...
namespace TraderModding
{
    public class TraderModdingScript : MonoBehaviour
    {
        ...
        public void UpdateModView()
        {
            ...
            EditBuildManipulation manip = new EditBuildManipulation(__instance.InventoryController, new CompoundItem[] { (CompoundItem)Globals.fakestashTraderController.RootItem }, playeritems_usable_mods, Globals.profile, __session.RagFair.Available);
        ...
        public async void TryToBuyItems()
        {
            ...
            ITradingSession traderInteractions = ClientAppUtils.GetMainApp().Session;
            ...
            foreach (var itemToBuyMongoID in Globals.itemsToBuy)
            {
                ...
                {
                    NotificationManager.DisplayWarningNotification("Skipping flea item: " + itemToBuyMongoID.LocalizedShortName(), EFT.Communications.ENotificationDurationType.Long);
                    continue;
                }
                ...
                {
                    NotificationManager.DisplayWarningNotification("Not enough money for: " + itemToBuyMongoID.LocalizedShortName(), EFT.Communications.ENotificationDurationType.Long);
                    continue;
                }
                ...
                if (buyItemTask.Result.Succeed)
                {
                    ...
                    NotificationManager.DisplayMessageNotification("Bought " + itemToBuyMongoID.LocalizedShortName() + " for " + costText, EFT.Communications.ENotificationDurationType.Long);
                }
            }
            ...
            foreach (string tplID in Globals.itemsToDetach)
            {
                ...
                if (bestCandidateToDetach != null)
                {
                    ...
                    if (!bestCandidateIsEquipped || TraderModdingConfig.DetachEquippedItems.Value)
                    {
                        ...
                        OperationResult<IItemOperationResult> moveOperationSimulation = ItemManipulator.QuickFindAppropriatePlace(bestCandidateToDetach, __instance.InventoryController, __instance.InventoryController.Inventory.Stash.ToEnumerable<Stash>(), ItemManipulator.EMoveItemOrder.TryTransfer, true);
                        ...
                        if (moveSuccess)
                            NotificationManager.DisplayMessageNotification("Detached " + detachText);
                        else
                            NotificationManager.DisplayWarningNotification("Detaching of " + detachText + " failed!");
                    }
                    else
                    {
                        NotificationManager.DisplayWarningNotification("Did not detach " + moveItemName + ", because it is equipped!");
                    }
                }
                else
                {
                    NotificationManager.DisplayWarningNotification("Could not find any appropriate item to detach!");
                }
            }
