...
{
    public Task OnLoadAsync(CancellationToken cancellationToken) { return Task.CompletedTask; }
...
public class TraderModdingRouter : StaticRouter
{
    ...
    private readonly TraderModdingCallback callback;

    public TraderModdingRouter(
        JsonUtil jsonUtil,
        TraderModdingCallback callback)
        : base(jsonUtil,
        [
            new RouteAction<EmptyRequestData>(
                "/choochoo-trader-modding/json",
                (url, info, sessionId, output, cancellationToken) =>
                    callback.GetTraderMods(sessionId, false)
            ),
            new RouteAction<EmptyRequestData>(
                "/choochoo-trader-modding/json-flea",
                (url, info, sessionId, output, cancellationToken) =>
                    callback.GetTraderMods(sessionId, true)
            )
        ])
    {
...
[Injectable]
public class TraderModdingCallback(
    ...
    JsonUtil jsonUtil)
{
    SPTarkov.Server.Core.Services.Server.PostDbLoadService postDbLoadService;
    
    public ValueTask<string> GetTraderMods(MongoId sessionId, bool get_flea_items)
    {  
        ...
        if (pmcData.Info != null && pmcData.Info.GameVersion != null &&
            globalTable.Configuration.TradingSettings.BuyRestrictionMaxBonus.TryGetValue(pmcData.Info.GameVersion, out var buyMaxBonus))
        {
            buyRestrictionMaxBonus = buyMaxBonus.Multiplier;
        }
        
        // Check for flea unlock
        bool fleaUnlocked = (pmcData.Info != null && pmcData.Info.Level != null && pmcData.Info.Level >= globalTable.Configuration.RagFair.MinUserLevel);
