...
namespace BeltSlot
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        private EEftScreenType getCurrentScreen()
        {
            ...
            EEftScreenType _eScreenType = currentScreenSingletonClass.CurrentBaseScreenController.ScreenType;
        ...
        // 4.1.2: ContainersPanel no longer has a static "equipmentSlot_0" field.
        // The active slot-name array is stored here and applied to the instance
        // field "_slotNames" by ContainersPanelPatch2 (Show prefix).
        internal static EquipmentSlot[] ActiveSlotNames { get; private set; }

        void SetEquipmentSlots()
        {
            if (Settings.BeltSlotLocation.Value == BeltSlotLocationOption.AbovePockets)
            {
                ActiveSlotNames = aboveEquipmentSlots;
            }
            else
            {
                ActiveSlotNames = belowEquipmentSlots;
            }
        }
        #endregion
        ...
        private void Awake()
        {
            ...
            // 4.1.2 port: PackNStrap integration (GetPrioritizedContainersPackNStrapPatch)
            // is excluded from this build because WTT-PackNStrap.dll is not present in the
            // 4.1.2 reference set. The vanilla loot-priority patch is always used.
