    ...
    class BossNotifierMono : MonoBehaviour {
        ...
        private void SendBossNotifications() {
            ...
            if (bossNotificationMessages.Count == 0) {
                NotificationManager.DisplayMessageNotification("No Bosses Located", ENotificationDurationType.Long, ENotificationIconType.Default, null);
                return;
            }

            foreach (var bossMessage in bossNotificationMessages) {
                NotificationManager.DisplayMessageNotification(bossMessage, ENotificationDurationType.Long, ENotificationIconType.Default, null);
            }
        }
        ...
        public void Update() {
            if (BotBossPatch.vicinityNotifications.Count > 0) {
                ...
                if (Instance.intelCenterLevel >= BossNotifierPlugin.intelCenterDetectedUnlockLevel.Value) {
                    NotificationManager.DisplayMessageNotification(notif, ENotificationDurationType.Long, ENotificationIconType.Default, null);
