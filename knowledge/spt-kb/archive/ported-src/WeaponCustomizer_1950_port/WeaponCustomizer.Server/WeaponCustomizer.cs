    ...
    JsonUtil jsonUtil)
{
    ...
    public async ValueTask<string> SaveCustomizations(SaveRequestData requestData, CancellationToken cancellationToken)
    {
    ...
    public async Task Save()
    {
        ...
        try
        {
            await fileUtil.WriteFileAsync(filePath, jsonUtil.Serialize(file, true), CancellationToken.None);
        }
        catch (Exception ex)
        {
    ...
    public async Task Load()
    {
        ...
        try
        {
            ...
            var json = await fileUtil.ReadFileAsync(filePath, CancellationToken.None);
    ...
    public async Task Clean()
    {
        ...
        if (dirtyCount > 0)
        {
            logger.Debug($"WeaponCustomizer: Cleaned up {dirtyCount} customizations for weapons/presets that no longer exist", null);
            await Save();
        }
    }
}
