...
public static class EditPatches
{
    ...
    {
        PresetField = AccessTools.GetDeclaredFields(typeof(EditBuildScreen)).Single(f => f.FieldType == typeof(WeaponBuild));
    ...
    public class RemoveBuildPatch : ModulePatch
    {
        ...
        {
            __instance.WeaponBuilds.TryGetValue(buildId, out __state);
        }
    ...
    public class ModdingIsActiveInRaidPatch : ModulePatch
    {
        ...
        {
            if (__instance.Weapon is null || Settings.ModifyRaidWeapons.Value == ModRaidWeapon.Never)
            {
                return;
            }
    ...
    public class ModdingIsInteractiveInRaidPatch : ModulePatch
    {
        ...
        [PatchPostfix]
        public static void Postfix(ItemContextInteractionsSwitcher __instance, EItemInfoButton button, ref IResult __result, ItemController ____itemController)
        {
            if (button == EItemInfoButton.Modding && Plugin.InRaid() && ____itemController is InventoryController inventoryController)
            {
                if (inventoryController.ID == __instance.Owner.ID && inventoryController.IsItemEquipped(__instance.Weapon))
                {
                    __result = new FailedResult("You can't edit equipped weapon", 0);
                    return;
                }

                if (Settings.ModifyRaidWeapons.Value == ModRaidWeapon.WithTool && !inventoryController.Inventory.Equipment.GetAllItems().Any(i => i.StringTemplateId == MultitoolId))
                {
                    __result = new FailedResult("Inventory Errors/Not moddable without multitool", 0);
                    return;
                }
