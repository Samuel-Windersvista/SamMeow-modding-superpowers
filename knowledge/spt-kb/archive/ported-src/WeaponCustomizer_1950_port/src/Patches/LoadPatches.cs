...
public static class LoadPatches
{
    ...
    public class MenuLoadPatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            Type type = PatchConstants.EftTypes.Single(
                t => !t.IsAbstract &&
                typeof(EFT.ClientBackendSession).IsAssignableFrom(t) &&
    ...
    public class InsuranceMessageReceivedPatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools.Method(typeof(SocialNetwork), nameof(SocialNetwork.MessageReceivedHandler));
        }

        [PatchPostfix]
        public static void Postfix(ChatShared.DialogueChatMessage message)
        {
