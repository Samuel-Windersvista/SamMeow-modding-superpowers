    ...
    public class ItemWeightPatch : ModulePatch
    {
        ...
        public static void PatchPostfix(Item __instance, ref float __result)
        {
            ...
            try
            {
                ...
                if (!_emptyWeightDB.TryGet(__instance.StringTemplateId, out float emptyWeight))
                {
                    emptyWeight = Mathf.Max(0.0f, Plugin.DefaultEmptyFraction.Value * original);
                    Logger.LogWarning($"Item '{__instance.LocalizedName()}' ({__instance.StringTemplateId}) not found in empty weight DB");
                }
