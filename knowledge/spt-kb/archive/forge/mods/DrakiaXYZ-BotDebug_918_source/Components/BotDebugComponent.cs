...
namespace DrakiaXYZ.BotDebug.Components
{
    internal class BotDebugComponent : MonoBehaviour, IDisposable
    {
        ...
        private void OnGUI()
        {
            ...
            {
                var debugBotStruct = bot.Value.Data;
                if (!debugBotStruct.InitedBotData) continue;
                ...
                if (screenPos.z > 0)
                {
                    if (updateGuiPending)
                    {
                        StringBuilder botInfoStringBuilder = BotInfo.GetInfoText(debugBotStruct, localPlayer, Settings.ActiveMode.Value);
        ...
        {
            public void SetData(DebugBotStruct botData)
            {
