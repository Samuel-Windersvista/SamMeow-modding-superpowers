...
- 目标框架：服务端 `net10.0` / 客户端 `netstandard2.1`
...
### STD-BUILD-002 — 客户端只使用 Mono 兼容目标框架（优先 `netstandard2.1`）

- **Level:** MUST
- **Applies:** both
- **Evidence:** 机制：`templates/client-mod/ClientModTemplate.csproj:5-9`（Unity Mono 无法加载 `net10.0` 程序集；官方客户端模块以 `netstandard2.1` 为目标）、`modding-guide/03-client-mod-anatomy.md`；语料：本次普查 264 个引用 BepInEx 的 csproj 中 227 个为 Mono 兼容框架（`netstandard2.1` 135、`net472` 79、`net471` 7 等），仅 1 个 `net10.0`（EV-CORPUS-CSPROJ）。
- **Rule:** 客户端 mod 的 `<TargetFramework>` 必须使用 Mono 兼容框架（优先 `netstandard2.1`，`net472`/`net471` 亦可），禁止 `net10.0`、`net9.0` 等 .NET Core/5+ 目标。
...
- **Level:** MUST
- **Applies:** both
- **Evidence:** 机制：`templates/client-mod/ClientModTemplate.csproj:29-51`（`HintPath` 指向 `$(SPTInstallPath)` 下 DLL 并设 `Private=false`）；语料：本次普查 264 个客户端 csproj 中 226 个使用 `<HintPath>`、111 个设 `<Private>false</Private>`（EV-CORPUS-CSPROJ）。
- **Rule:** 客户端引用 BepInEx、Harmony、`Assembly-CSharp`、UnityEngine 等运行时程序集时，必须通过 `<HintPath>` 指向 `$(SPTInstallPath)` 下的实际 DLL，并设 `<Private>false</Private>`，不得把这些 DLL 复制进构建输出。
...
- **Level:** MUST
- **Applies:** both
- **Evidence:** 机制：`modding-guide/01-environment-toolchain.md`（引用包 `SPTarkov.Server.Core` 版本必须与服务器匹配）、`api-notes-5.0/mod-loading.md`（mod 引用的 Core 版本高于运行时即抛异常，先于其它校验）、`templates/server-mod/ServerModTemplate.csproj:27-45`；语料：本次普查 114 个引用 `SPTarkov.Server` 的 csproj 中 83 个用 NuGet `PackageReference`、31 个用 `<HintPath>`/本地 `<Reference>`（EV-CORPUS-CSPROJ）。
- **Rule:** 服务端 mod 必须引用 `SPTarkov.Server.Core`（及 `SPTarkov.DI`、`SPTarkov.Common`），且引用版本不得高于目标运行时版本；优先使用 NuGet `PackageReference` 并锁定与目标 SPT 一致的版本号。
...
### STD-BUILD-006 — 用可覆盖的 `SPTInstallPath` 定位安装目录

- **Level:** SHOULD
- **Applies:** both
- **Evidence:** 机制：`templates/server-mod/ServerModTemplate.csproj:19`、`templates/client-mod/ClientModTemplate.csproj:22`（`Condition="'$(SPTInstallPath)' == ''"`，允许命令行覆盖）；语料：`skills/writing-spt-mod/SKILL.md` 将「硬编码路径」列为反模式（`EV-CORPUS-MATERIALS`）。
- **Rule:** 用 `SPTInstallPath` 属性定位 SPT 安装目录，并允许 `dotnet build -p:SPTInstallPath=...` 覆盖；不要在 csproj 中硬编码某一台机器的绝对路径。
